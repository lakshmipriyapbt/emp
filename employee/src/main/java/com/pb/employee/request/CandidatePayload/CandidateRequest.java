...
import com.pb.employee.validations.ValidCandidateAge;
...
@NoArgsConstructor
@AllArgsConstructor
@ValidCandidateAge // <-- Apply the custom validation here
public class CandidateRequest {
    ...
    @Schema(example = "candidateType")
    @Pattern(regexp = "^(?!.*\\b([A-Z])\\s\\1\\s\\1)(?:[A-Z][a-z]+(?: [A-Z][a-z]+)*|[A-Z](?:\\.? ?[A-Z])? ?[A-Z][a-z]+)$", message = "{candidate.type}")
    @Size(min = 3, max = 20, message = "{candidateType.size.message}")
